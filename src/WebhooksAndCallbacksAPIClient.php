...
use WebhooksAndCallbacksAPILib\Apis\OrdersApi;
...
use WebhooksAndCallbacksAPILib\Authentication\ApiKeyManager;
...
use WebhooksAndCallbacksAPILib\Authentication\BearerAuthManager;
...
use WebhooksAndCallbacksAPILib\Utils\CompatibilityConverter;

class WebhooksAndCallbacksAPIClient implements ConfigurationInterface
{
    private $orders;
    ...
    /**
     * @see WebhooksAndCallbacksAPIClientBuilder::init()
     * @see WebhooksAndCallbacksAPIClientBuilder::build()
     *
     * @param array $config
     */
    public function __construct(array $config = [])
    {
        ...
        $this->client = ClientBuilder::init(
            ...
            ->userAgent('PHP-SDK/4.0.0 (OS: {os-info}, Engine: {engine}/{engine-version})')
            ->serverUrls(self::ENVIRONMENT_MAP[$this->getEnvironment()], Server::DEFAULT_)
            ->authManagers(['ApiKey' => $this->apiKeyManager, 'BearerAuth' => $this->bearerAuthManager])
     ...
     *
     * @return WebhooksAndCallbacksAPIClientBuilder WebhooksAndCallbacksAPIClientBuilder instance
     */
    public function toBuilder(): WebhooksAndCallbacksAPIClientBuilder
    {
        $builder = WebhooksAndCallbacksAPIClientBuilder::init()
    ...
    {
        if (empty($this->apiKeyManager->getXAPIKey())) {
            return null;
        }
        return ApiKeyCredentialsBuilder::init($this->apiKeyManager->getXAPIKey());
    }
     ...
     *
     * @see WebhooksAndCallbacksAPIClientBuilder::getConfiguration()
     */
    ...
    private const ENVIRONMENT_MAP =
        [Environment::PRODUCTION => [Server::DEFAULT_ => 'http://localhost:3000/callbacks']];
}
